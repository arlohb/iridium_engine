...
fn main() {
    ...
    assets.add("sprite_vertex", Shader::new(
        &app.device,
        ShaderType::Vertex,
    ...
    assets.add("sprite_fragment", Shader::new(
        &app.device,
        ShaderType::Fragment,
        include_spirv!("src/sprite.hlsl", frag, hlsl, entry="fs_main"),
        vec![
            assets.get::<Texture>("steak_tex").unwrap().texture_binding_type,
            assets.get::<Texture>("steak_tex").unwrap().sampler_binding_type,
        ],
    ));
    assets.add("uv_test_fragment", Shader::new(
        &app.device,
        ShaderType::Fragment,
    ...
    assets.add("steak_mat", Material::new(
        &app.device,
    ...
    assets.add("uv_test", Material::new(
        &app.device,
        app.surface_config.format,
        assets.get::<Shader>("sprite_vertex").unwrap(),
        world.entities.get::<Renderer2DState>().camera_gpu_data.as_ref().unwrap(),
